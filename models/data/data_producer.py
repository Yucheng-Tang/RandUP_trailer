...
class RandUP:
    def __init__(self):
        ...
        self.U_max = [self.v_limit, np.deg2rad(self.ang_limit)]
    ...
    def randup_trailer(vehicle, randup_process, polygon_operator):
        ...
        for i_x in range(len(randup_process.x)):
            ...
            obstacle_vertices = np.array([[-0.5, 0], [-0.7, 0.3], [-1, 0], [-0.7, -0.3]])
            obstacle_vertices_random = obstacle_vertices + np.random.uniform(low=-0.2, high=0.2, size=(1, 2))
            ...
            # plot
            RandUP.plot_sets(vehicle, randup_process,  reachable_sets, obstacles, intersection_sets, sampling_reach_points, index=i_x)
        return reachable_sets, obstacles, intersection_sets, sampling_reach_points
...
def output_excel(initial_states, obstacles, intersection_sets):
    wb = Workbook()
...
def main():
    ...
    reachable_sets, obstacles, intersection_sets, sampling_reach_points = RandUP.randup_trailer(vehicle,
                                                                                                randup_process,
                                                                                                polygon_operator)
    ...
    # output_excel(initial_states, obstacles, intersection_sets)
